"""
CAMELS-H forecasting preprocessing: hourly parquet -> prepped.npz.

Reads the raw parquet (one row per basin-hour), builds cumulative
meteorological features, splits train/val/test by date, standardizes X
globally and Y per basin, and saves everything the training pipeline needs
into a single npz file.

Usage:
    python preprocess_camelsh_forecast.py [--parquet PATH] [--out PATH] [basin_id ...]
"""
...
def load_parquet_streaming(parquet_path, dynamic_vars, static_vars, target_var,
                            select_basins=None):
    """
    Stream the parquet row group by row group and assemble per-basin arrays.
    ...
    # Row groups are not one-per-basin: the parquet is a flat table sorted by
    # (basin_id, Time) cut into fixed-size row groups, so a row group can span
    # several basins and a basin can straddle row-group boundaries. Rows are
    # therefore grouped by basin_id and aligned onto a shared time axis by
    # timestamp.
    ...
    # Pass 2: shared time axis = timestamps of the first basin in row group 0
    ...
    n_times = len(times)
    print(f"Time axis: {n_times} steps "
...
def rolling_sum_min1(arr, window):
    ...
    Uses a float64 accumulator, cast back to float32.
    """
...
def build_cumulative_features(dynamic):
    """
    Returns dict {name: (n_time, n_basin) float32} with features:
...
def build_valid_mask(x_vars, dynamic, static, cumul, n_times, n_basins):
    """(n_time, n_basin) bool: True where all X features are non-NaN."""
...
def compute_y_scaling_per_basin(target, train_idx, n_basins):
    """Per-basin mean/std over non-NaN training values."""
...
def create_distance_matrix(static, n_basins):
    """Pairwise basin distances (euclidean on lat/lon, standardized, sigmoid)."""
...
def prep_data(parquet_file, out_file, select_basins=None):
    ...
    # Free large intermediates before the npz save
...
if __name__ == "__main__":
    ...
    if "--parquet" in args:
        i = args.index("--parquet")
        parquet_file = args[i + 1]
        args = args[:i] + args[i + 2:]
