...
class PICP(Metric):
    """Prediction Interval Coverage Probability for a given percentile range."""
    ...
    def update(self, all_gen_y: torch.Tensor, y_true: torch.Tensor):
        all_gen_y = all_gen_y.view(-1, all_gen_y.shape[3]).cpu()  # (B * O * N, S)
        y_true = y_true.view(-1).cpu()                             # (B * O * N,)
