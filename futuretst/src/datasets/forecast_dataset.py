...
class ForecastDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Cycle basins first, then time offsets, so batch_size=n_basins
        # yields all basins at the same time window
        ...
        # Condition = [X, (Y_imputed), Y_history]; Y_history stays last so
        # models can rely on batch_x[:, :, -1:] being the target history
        if self.has_imputed:
