"""
FutureTST: encoder-decoder Transformer for time-series forecasting with
known future exogenous inputs.

The endogenous history (target variable) is split into patches and fed to the
decoder; the exogenous features (history + future) are projected along the
time axis and fed to the encoder. Cross-attention lets the decoder condition
its predictions on the full exogenous window, including the forecast horizon.

Input/output convention:
    forward(x):
        x.shape == (B, T, C)
            B = batch (typically one entry per basin)
            T = context_window_size + pred_size
            C = input_channels (last channel is the endogenous target Y)
            x[:, :context_window_size, -1:]  -> Y history      (B, context, 1)
            x[:, :, :-1]                     -> exogenous X    (B, T, C-1)
    returns:
        predictions of shape (B, pred_size, 1)

No per-window instance normalization is applied; inputs are expected to be
z-scored by the preprocessing pipeline, and outputs stay in that scale.
"""
import math
import numpy as np
import torch
...
class ExtractPatches(nn.Module):
    """
    Slice the time axis (dim=1) into overlapping patches.

    Input  (B, T, C)
    Output (B, num_patches, patch_size, C)

    The last time step is repeated stride_len times as tail padding so the
    final patch is complete.
    """
...
class LinearProjectionLayer(nn.Module):
    """
    Project each patch (patch_size time steps) to d_model, per channel.

    Input  (B, num_patches, patch_size, C)
...
class LinearHead(nn.Module):
    """Flatten (B, num_patches, d_model) and project to (B, pred_size)."""
...
class PositionalEncoding(nn.Module):
    """Sinusoidal positional encoding applied to (B, seq_len, d_model)."""
...
class FutureTST(nn.Module):
    """
    Parameters:
        context_window_size : history length
        patch_size          : patch window size
        stride_len          : stride between patches (also tail padding length)
        d_model             : Transformer hidden dimension
        num_transformer_layers
        mlp_size            : feed-forward hidden dimension
        num_heads
        mlp_dropout
        pred_size           : forecast horizon
        embedding_dropout
        input_channels      : total input channels = n exogenous + 1 (endogenous Y)
    """
    ...
    def __init__(self, context_window_size: int = 365, patch_size: int = 16, stride_len: int = 8,
        ...
        # Exogenous features are projected along the full time axis (context + horizon)
        ...
        # Exogenous PE runs over channels, so seq_len = input_channels - 1
    ...
    def forward(self, x: torch.Tensor):
        ...
        x: (B, T, C), T = context_window_size + pred_size, C = input_channels
            x[:, :, :-1]                     = exogenous X (history + future)  (B, T, C-1)
            x[:, :context_window_size, -1:]  = endogenous Y history            (B, context, 1)
        """
        exogeneous_data = x[:, :, :-1]                                          # (B, T, C-1)
        endogeneous_data = x[:, :self.context_window_size, -1:]                 # (B, context, 1)

        # Endogenous: patch, project, add positional encoding
        ...
        endo_for_pe = self.endo_positional_encoding(endo_for_pe)

        # Exogenous: project the time axis to d_model per channel
        exo_t = exogeneous_data.permute(0, 2, 1)                                # (B, C-1, T)
        exo_proj = self.exogeneous_feature_projection(exo_t)                    # (B, C-1, d_model)
        exo_for_pe = self.exo_positional_encoding(exo_proj)

        # Encoder over exogenous channels, decoder over endogenous patches
        ...
        return predictions.unsqueeze(-1)                                        # (B, pred_size, 1)
